 ...
 */
  struct L2
  {
    ...
    double compute_distance(
      const ElementType & elema, const ElementType2 & elemb,
...
public:
    ...
    inline std::vector < std::pair < uint32_t, double >> searchKnn(
    ...
    template < typename Type, typename Container >
    inline std::vector < std::pair < uint32_t, double >> radiusSearch(
      const Container & container,
...
private:
    ...
    template < typename Container >
    void divideTree(
      Index & index, uint64_t nodeIdx, int startIndex, int endIndex,
      BoundingBox & bbox, const Container & container)
    {
      ...
      if (_maxLeafSize != 1) {
        if (split_index < _maxLeafSize || count - split_index < _maxLeafSize) {
          std::sort(
            all_indices.begin() + startIndex, all_indices.begin() + endIndex,
    ...
    template < typename Container >
    void mean_var_calculate(
      int startindex, int endIndex, double var[], double mean[],
      const Container & container)
    {
    ...
    template < typename Container >
    void planeSplit(
      uint32_t * ind, int count, int cutfeat, float cutval, int & lim1,
    ...
    inline double computeInitialDistances(
      const Type & elem, double dists[],
...
public:
      ResultSet(
        std::vector < std::pair < uint32_t, double >> &data_ref,
...
private:
    ...
    template < typename Type, typename Container >
    inline void searchExactLevel(
      const Index & index, int64_t nodeIdx, const Type & elem,
      ...
      if (currNode.isLeaf()) {
        ...
        for (size_t i = 0; i < currNode.idx.size(); i++) {
          double sqd = _distance.compute_distance(
            elem, container.at(currNode.idx[i]),
