...
public:
    ...
    std::vector < aruco::Marker > detect(
      const cv::Mat & input, const CameraParameters & camParams,
...
private:
    ...
    void distortPoints(
      std::vector < cv::Point2f > in, std::vector < cv::Point2f > & out,
    ...
    template < typename T >
    void joinVectors(
      std::vector < std::vector < T >> & vv, std::vector < T > & v,
      bool clearv = false)
    {
      if (clearv) {
        v.clear();
    ...
    std::vector < aruco::MarkerCandidate > thresholdAndDetectRectangles(
      const cv::Mat & input,
      int thres_param1,
    ...
    std::vector < aruco::MarkerCandidate > prefilterCandidates(
...
private:
    ...
    void refineCornerWithContourLines(
      aruco::Marker & marker, cv::Mat cameraMatrix = cv::Mat(),
