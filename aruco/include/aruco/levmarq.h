...
public:
    ...
    typedef std::function <
      void(const eVector &, Eigen::Matrix < T, Eigen::Dynamic, Eigen::Dynamic > &) > F_z_J;
    LevMarq();
    /**
     ...
     */
    LevMarq(
      int maxIters, double minError, double min_step_error_diff = 0, double tau = 1,
     ...
     */
    void setParams(
      int maxIters, double minError, double min_step_error_diff = 0,
    ...
    void calcDerivates(
      const eVector & z, Eigen::Matrix < T, Eigen::Dynamic, Eigen::Dynamic > &,
      F_z_x);
 ...
 */
  template < typename T >
  LevMarq < T > ::LevMarq(
    int maxIters, double minError, double min_step_error_diff, double tau,
    double der_epsilon)
  {
    _maxIters = maxIters;
 ...
 */
  template < typename T >
  void LevMarq < T > ::setParams(
    int maxIters, double minError, double min_step_error_diff,
  ...
  double LevMarq < T > ::solve(eVector & z, F_z_x f_z_x)
  {
    return solve(
      z, f_z_x,
      std::bind(
        &LevMarq::calcDerivates, this, std::placeholders::_1,
