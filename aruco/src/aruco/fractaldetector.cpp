...
void FractalDetector::draw3d(cv::Mat & img, bool cube, bool axis)
{
  if (Tracker.isPoseValid()) {
    ...
    if (innerPoints3d.size() > 0) {
      ...
      projectPoints(
        innerPoints3d, Tracker.getRvec(), Tracker.getTvec(),
    ...
    // Draw axes
    if (axis) {
      CvDrawingUtils::draw3dAxis(
        img, _cam_params, getRvec(), getTvec(),
        Tracker.getFractal().getFractalSize() / 2);
    }
  }
}

void FractalDetector::draw3dCube(
  cv::Mat & Image, FractalMarker m,
