...
void FractalPoseTracker::setParams(
...
bool FractalPoseTracker::fractalRefinement(
  ...
  int markerWarpPix)
{
  ...
  for (auto id_marker : _fractalMarker.fractalMarkerCollection) {
    ...
    if (marker3d.size() < 4) {
      ...
      cv::projectPoints(
        _inners3d, _rvec, _tvec, _cam_params.CameraMatrix,
        _cam_params.Distorsion, _inners2d);
    } else {
      std::vector<cv::Point2f> marker2d;
      cv::projectPoints(
        marker3d, _rvec, _tvec, _cam_params.CameraMatrix,
    ...
    if (ratio == 1 && area >= desiredarea) {
      ...
      int idx = 0;
      for (auto pt : _inners2d) {
        _inners2d_error.push_back(
          sqrt(
            pow(_inners2d_copy[idx].x - pt.x, 2) +
  ...
  if (_ref_inner3d.size() > 4) {
    aruco::solvePnP(
      _ref_inner3d, _ref_inner2d, _cam_params.CameraMatrix,
...
//#define _fractal_debug_inners
    ...
    cv::projectPoints(
      _inners, _rvec, _tvec, _cam_params.CameraMatrix,
...
bool FractalPoseTracker::fractalInnerPose(
  const cv::Ptr<aruco::MarkerDetector> markerDetector,
  ...
  if (vmarkers.size() > 0) {
    ...
    if (!_rvec.empty()) {
      ...
      if (innerPoints3d.size() > 0 && radius > 0) {
        cv::projectPoints(
          innerPoints3d, _rvec, _tvec, _cam_params.CameraMatrix,
...
//#define _fractal_debug_classification
          ...
          if (!bestModel.empty()) {
            ...
            if (p3d.size() >= 4) {
              ...
              aruco::solvePnP(
                p3d, p2d, _cam_params.CameraMatrix, _cam_params.Distorsion,
                _rvec, _tvec);
              //                                Timer.add("solves");
  ...
  float & ratio)
{
  ...
  if (!biggest_p3d.empty()) {
    cv::projectPoints(
      biggest_p3d, _rvec, _tvec, _cam_params.CameraMatrix,
    ...
    cv::projectPoints(
      smallest_p3d, _rvec, _tvec, _cam_params.CameraMatrix,
...
void FractalPoseTracker::drawKeyPoints(
  ...
  bool text, bool transf)
{
  ...
  if (text) {
    ...
    for (auto kp : kpoints) {
      putText(
        out, std::to_string(nkm++), kp.pt, CV_FONT_HERSHEY_COMPLEX, 1,
