...
std::vector<aruco::Marker> MarkerDetector_Impl::detect(
  const cv::Mat & input,
 ...
 *
 ************************************/
void MarkerDetector_Impl::detect(
  ...
  bool setYPerpendicular, bool correctFisheye)
{
  if (camParams.CamSize != input.size() && camParams.isValid() && markerSizeMeters > 0) {
    ...
    detect(
      input, detectedMarkers, cp_aux.CameraMatrix, cp_aux.Distorsion,
      cp_aux.ExtrinsicMatrix, markerSizeMeters, setYPerpendicular, correctFisheye);
  } else {
    detect(
      input, detectedMarkers, camParams.CameraMatrix, camParams.Distorsion,
...
void MarkerDetector_Impl::buildPyramid(
  vector<cv::Mat> & ImagePyramid, const cv::Mat & grey,
  int minSize)
{
  ...
  const cv::Mat & input, int thres_param1, int thres_param2, bool enclosed, cv::Mat & auxThresImage)
{
  ...
  if (_params.thresMethod == MarkerDetector::THRES_AUTO_FIXED) {
    ...
    if (enclosed) {
      ...
      cv::erode(
        ...
        getStructuringElement(
          MORPH_CROSS, cv::Size(enclosedOffset, enclosedOffset),
    ...
    cv::adaptiveThreshold(
      input, auxImage, 255., ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY_INV,
...
void MarkerDetector_Impl::thresholdAndDetectRectangles_thread()
{
  while (true) {
    ...
    _vcandidates[tad.outIdx] =
      thresholdAndDetectRectangles(
      _thres_Images[tad.inIdx], tad.param1, tad.param2,
      enclosed, _thres_Images[tad.outIdx]);
  }
}

vector<aruco::MarkerDetector_Impl::MarkerCandidate> MarkerDetector_Impl::
thresholdAndDetectRectangles(
  const cv::Mat & image)
{
  ...
  bool correctFisheye)
{
  ...
  do{
    ...
    for (size_t i = 0; i < MarkerCanditates.size(); i++) {
      ...
      if (markerIdDetector->detect(canonicalMarkerAux, id, nRotations, additionalInfo)) {
        ...
        std::rotate(
          detectedMarkers.back().begin(), detectedMarkers.back().begin() + 4 - nRotations,
  ...
  if (_params.trackingMinDetections > 0) {
    ...
    if (needsTrack.size() > 0) {
      ...
      _candidates_wcontour.erase(
        std::remove_if(
          _candidates_wcontour.begin(), _candidates_wcontour.end(),
  ...
  if (detectedMarkers.size() > 0 && input.size() == imgToBeThresHolded.size()) {
    if (_params.cornerRefinementM == CORNER_SUBPIX) {
      ...
      cornerSubPix(
        grey, Corners, cv::Size(halfwsize, halfwsize), cv::Size(-1, -1),
  ...
  if (camMatrix.rows != 0 && markerSizeMeters > 0) {
    for (unsigned int i = 0; i < detectedMarkers.size(); i++) {
      detectedMarkers[i].calculateExtrinsics(
        markerSizeMeters, camMatrix, distCoeff,
...
void MarkerDetector_Impl::refineCornerWithContourLines(
  aruco::Marker & marker,
...
/**
 */
void MarkerDetector_Impl::interpolate2Dline(
...
void MarkerDetector_Impl::drawApproxCurve(
  Mat & in, vector<Point> & contour, Scalar color,
  int thickness)
{
  for (unsigned int i = 0; i < contour.size(); i++) {
...
/**
 */
void MarkerDetector_Impl::distortPoints(
  vector<cv::Point2f> in, vector<cv::Point2f> & out,
...
void MarkerDetector_Impl::cornerUpsample(
  std::vector<Marker> & MarkerCanditates,
  cv::Size lowResImageSize)
{
  ...
  for (int curpyr = startPyrImg; curpyr >= 0; curpyr--) {
    ...
    cv::cornerSubPix(
      imagePyramid[curpyr], p2d, cv::Size(halfwsize, halfwsize),
