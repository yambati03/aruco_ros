...
std::vector<std::pair<cv::Mat, double>> solvePnP_(
  float size,
  const std::vector<cv::Point2f> & imgPoints,
  ...
  cv::InputArray distCoeffs)
{
  ...
  IPPE::PoseSolver Solver;
  Solver.solveSquare(
    size, imgPoints, cameraMatrix, distCoeffs, Rvec, Tvec, reprojErr1,
...
void IPPE::PoseSolver::solveGeneric(
  cv::InputArray _objectPoints, cv::InputArray _imagePoints,
  ...
  cv::OutputArray _tvec2, float & err2)
{
  ...
  sortPosesByReprojError(
    _objectPoints, _imagePoints, _cameraMatrix, _distCoeffs, Ma, Mb,
    M1, M2, err1, err2);
...
void IPPE::PoseSolver::solveCanonicalForm(
  cv::InputArray _canonicalObjPoints,
  cv::InputArray _normalizedInputPoints,
...
void IPPE::PoseSolver::solveSquare(
  double squareLength, InputArray _imagePoints,
  ...
  OutputArray _rvec2, OutputArray _tvec2, float & err2)
{
  ...
  sortPosesByReprojError(
    objectPoints3D, normalizedInputPoints, _cameraMatrix,
...
double IPPE::PoseSolver::meanSceneDepth(
  InputArray _objectPoints, InputArray _rvec,
  InputArray _tvec)
{
...
void IPPE::PoseSolver::computeTranslation(
  InputArray _objectPoints, InputArray _normalizedImgPoints,
...
void IPPE::PoseSolver::computeRotations(
...
void IPPE::PoseSolver::homographyFromSquarePoints(
  InputArray _targetPoints,
...
void IPPE::PoseSolver::makeCanonicalObjectPoints(
  InputArray _objectPoints,
...
void IPPE::PoseSolver::evalReprojError(
  cv::InputArray _objectPoints, cv::InputArray _imagePoints,
  cv::InputArray _cameraMatrix, cv::InputArray _distCoeffs,
  cv::InputArray _M, float & err)
{
  ...
  if (_cameraMatrix.empty()) {
    ...
    cv::Mat kc;
    cv::projectPoints(
      _objectPoints, r, _M.getMat().colRange(3, 4).rowRange(0, 3), K, kc,
      projectedPoints);
  } else {
    cv::projectPoints(
      _objectPoints, r, _M.getMat().colRange(3, 4).rowRange(0, 3),
...
void IPPE::PoseSolver::sortPosesByReprojError(
  cv::InputArray _objectPoints,
...
void HomographyHO::normalizeDataIsotropic(
  cv::InputArray _Data, cv::OutputArray _DataN,
  cv::OutputArray _T, cv::OutputArray _Ti)
{
  ...
  assert(
    (dataType == CV_64FC2) | (dataType == CV_64FC3) | (dataType == CV_32FC2) |
    (dataType == CV_32FC3));
...
void HomographyHO::homographyHO(
  cv::InputArray _srcPoints, cv::InputArray _targPoints,
  cv::OutputArray _H)
{
