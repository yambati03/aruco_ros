...
void Dictionary::fromVector(
  const vector<uint64_t> & codes, std::map<uint64_t,
  uint16_t> & code_id_map)
{
  code_id_map.clear();
  int id = 0;
  for (auto c : codes) {
...
Dictionary Dictionary::loadPredefined(DICT_TYPES type)
{
  ...
  switch (type) {
    ...
    case CUSTOM:
      throw cv::Exception(
              -1, "CUSTOM type is only set by loading from file",
    ...
    default:
      throw cv::Exception(
              9001, "Invalid Dictionary type requested",
...
cv::Mat Dictionary::getMarkerImage_id(
  int id, int bit_size, bool addWaterMark,
  ...
  bool centralCircle)
{
  ...
  for (int y = nBitsSquared - 1; y >= 0; y--) {
    for (int x = nBitsSquared - 1; x >= 0; x--) {
      if (bset[bidx++]) {
        cv::Mat bit_pix = img(
          cv::Range((1 + y) * bit_size, (2 + y) * bit_size),
  ...
  if (addWaterMark) {
    ...
    cv::putText(
      img, idcad, cv::Point(0, img.rows - img.rows / 40),
  ...
  if (enclosed_corners) {
    ...
    biggerImage(
      cv::Range(biggerImage.rows - sy, biggerImage.rows),
  ...
  if (externalWhiteBorder && !enclosed_corners) {
    ...
    biggerImage.setTo(cv::Scalar::all(255));
    // set the image in the center
    cv::Mat center = biggerImage(
      cv::Range(borderSize, borderSize + img.rows),
...
MarkerMap Dictionary::createMarkerMap(
  ...
  const std::vector<int> & ids, bool chess_board)
{
  if (gridSize.height * gridSize.width != int(ids.size())) {
    throw cv::Exception(
            9001, "gridSize != ids.size()Invalid ",
      ...
      for (int x = 0; x < gridSize.width; x++) {
        ...
        if (toWrite) {
          if (CurMarkerIdx >= int(ids.size())) {
            throw cv::Exception(
                    999, " FiducidalMarkers::createMarkerMapImage_ChessMarkerMap",
